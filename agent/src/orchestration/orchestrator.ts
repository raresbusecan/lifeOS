...
import type {
  TestResult,
} from "../workflow/testing.js";

export interface OrchestrationOptions {
  repositoryRoot: string;
  maxCodingSteps?: number;
  timeoutMs?: number;
}
...
function createContractFromAnalysis(
  task: Task,
  planner: AgentOutput,
  analyst: AgentOutput,
): TaskContract {
...
export async function orchestrateTask(
  ...
  const planner =
    await runAgent({
      ...
      input: buildAgentInput(
        currentTask,
        "Plan the task and establish the intended outcome.",
      ),
  ...
  if (
    ...
    throw new Error(
      `Planner blocked the task: ${planner.findings.join("; ")}`,
    );
  }
  ...
  if (
    analyst.status === "BLOCKED" ||
    analyst.status === "FAILED"
  ) {
    throw new Error(
      `Analyst blocked the task: ${analyst.findings.join("; ")}`,
    );
  }
  ...
  if (currentTask.status === "PROPOSED") {
    ...
    store.transition(
      currentTask.id,
      "COUNCIL",
      "Task entered Council orchestration.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Architect blocked the task: ${architect.findings.join("; ")}`,
    );
  }
   ...
   */
  const contract =
    createContractFromAnalysis(
      currentTask,
      planner,
      analyst,
    );

  workflow.attachContract(
    contract,
  );
  ...
  currentTask =
    store.transition(
      currentTask.id,
      "CONTRACT_READY",
      "Task Contract approved by orchestration.",
    );
  ...
  currentTask =
    store.transition(
      currentTask.id,
      "IMPACT_APPROVED",
      "Impact Map validated and approved.",
    );
  ...
  if (
    ...
    throw new Error(
      `Git agent blocked the task: ${git.findings.join("; ")}`,
    );
  }
      ...
      [
        {
          role: "system",
          content:
            "You are the Coder for LifeOS. Implement the task using repository tools. Respect the Task Contract and Impact Map. Actually modify the repository.",
        },
        {
          role: "user",
          content:
  ...
  currentTask =
    store.transition(
      currentTask.id,
      "TESTING",
      "Implementation submitted for testing.",
    );
  ...
  if (
    testerOutput.status === "BLOCKED" ||
    testerOutput.status === "FAILED"
  ) {
    throw new Error(
      `Tester failed to produce a valid testing decision: ${testerOutput.findings.join("; ")}`,
    );
  }

  const tester =
    parseJson<TestResult>(
      testerOutput.findings[0] ?? "",
      "Tester",
    );

  const workflowTesting =
    workflow.handleTestResult(
      currentTask.id,
      tester,
    );
