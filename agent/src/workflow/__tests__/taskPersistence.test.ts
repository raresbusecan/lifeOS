...
try {
  ...
  const task = createTask({
    ...
    description:
      "Verify that workflow state survives a restart.",
    status: "READY",
    scope: {
  ...
  const childTask = createChildTask(
    task,
    {
      ...
      description:
        "Track a separately discovered concern.",
      scope: {
  ...
  assert.deepEqual(
    restored.getHistory(task.id),
    store.getHistory(task.id),
  );
  assert.deepEqual(
    restored.getChildren(task.id),
    [childTask],
  );

  assert.equal(
    getTaskStoreFile(repositoryRoot).endsWith(
      ".agent/runtime/tasks.json",
    ),
    true,
  );

  const emptyStore = await loadTaskStore(
    join(repositoryRoot, "new-repository"),
  );
