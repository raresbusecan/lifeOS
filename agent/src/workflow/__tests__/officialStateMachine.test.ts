...
function createOfficialTask() {
  return createTask({
    ...
    description:
      "Verify the workflow states defined in the development plan.",
    status: "CREATED",
  });
}

function moveThrough(
  statuses: Parameters<typeof moveTask>[1][],
) {
...
assert.throws(
  () => moveTask(createOfficialTask(), "CODING"),
  /Invalid task transition: CREATED -> CODING/,
);
