...
const passTask =
  createTask({
    ...
    title:
      "Coordinator PASS test",
    description:
      "Verify coordinator executes the PASS workflow.",
    status: "READY",
    scope: {
      files: [
        "src/auth/login.ts",
...
const retrievedPassTask =
  passCoordinator.getTask(
    passTask.id,
  );

assert.equal(
  retrievedPassTask.id,
  passTask.id,
);

assert.equal(
  retrievedPassTask.status,
  "READY",
);
...
const passOutcome =
  passCoordinator.handleTestResult(
    passTask.id,
    {
      type: "PASS",
      relatedTaskId:
...
const reworkTask =
  createTask({
    ...
    title:
      "Coordinator REWORK test",
    description:
      "Verify coordinator executes an in-scope rework.",
    status: "READY",
    scope: {
      files: [
        "src/auth/login.ts",
...
const reworkCoordinator =
  new WorkflowCoordinator(
    reworkStore,
  );

const reworkRun =
  reworkCoordinator.startCoding(
    reworkTask.id,
  );
...
const reworkOutcome =
  reworkCoordinator.handleTestResult(
    reworkTask.id,
    {
      type: "REWORK",
      relatedTaskId:
...
const childTask =
  createTask({
    ...
    title:
      "Coordinator NEW_TASK test",
    description:
      "Verify coordinator creates a child task for an out-of-scope finding.",
    status: "READY",
    scope: {
      files: [
        "src/auth/login.ts",
...
const childOutcome =
  childCoordinator.handleTestResult(
    childTask.id,
    {
      type: "NEW_TASK",
      relatedTaskId:
      ...
      details:
        "The tester discovered a database issue outside the original task scope.",
      expectedBehavior:
...
console.log(
  "Workflow coordinator integration test passed",
);