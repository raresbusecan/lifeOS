...
console.log(
    "Indexing contextual query repository...",
  );
...
assert.ok(
  semanticIndex.chunks > 0,
);

assert.equal(
  semanticIndex.vectorsIndexed,
  ...
  const authSource =
    second.sources.find(
      (source) =>
        source.path === "src/auth.ts",
    );

  assert.ok(
    authSource,
    "Expected contextual search to find src/auth.ts",
  );
  ...
  for (
    const [index, source]
    of second.sources.entries()
  ) {
    console.log(
      `${index + 1}. ${source.path}:${source.startLine}-${source.endLine} — ${source.score.toFixed(4)}`,
    );
  }

  console.log("");
  console.log(
    "Contextual query integration test passed",
  );
} finally {
  await rm(
    repositoryRoot,
