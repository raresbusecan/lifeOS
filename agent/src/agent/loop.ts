...
function toOllamaTools(): OllamaToolDefinition[] {
          ...
          parameters: {
            ...
            properties: {
              path: {
                type: "string",
                description:
                  "Repository-relative directory path to list. Defaults to the repository root.",
              },
            },
          },
        },
          ...
          parameters: {
            ...
            properties: {
              path: {
                type: "string",
                description:
                  "Repository-relative path of the file to create or overwrite.",
              },
              content: {
                type: "string",
                description:
...
function parseTextToolCalls(
  ...
  const functionRegex =
    /<function=([a-zA-Z0-9_-]+)>([\s\S]*?)(?:<\/function>|<\/tool_call>|$)/g;

  for (
    const match of content.matchAll(functionRegex)
  ) {
    const name = match[1];
...
export async function runAgentLoop(
  ...
  throw new Error(
    `Agent exceeded maximum tool steps (${maxSteps})`,
  );
}