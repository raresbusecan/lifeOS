...
assert.equal(passHistory[0]?.from, "GIT_READY");
...
assert.equal(passHistory[1]?.to, "IMPLEMENTED");
assert.equal(passHistory[1]?.reason, "Coding phase completed.");
assert.equal(passHistory[2]?.from, "IMPLEMENTED");
assert.equal(passHistory[2]?.to, "TESTING");
assert.equal(passHistory[2]?.reason, "Task moved to TESTING.");
...
const passFinalHistory = passStore.getHistory(passTask.id);
assert.equal(passFinalHistory[3]?.from, "TESTING");
assert.equal(passFinalHistory[3]?.to, "TRIAGE");
assert.equal(passFinalHistory[4]?.from, "TRIAGE");
assert.equal(passFinalHistory[4]?.to, "REVIEW");
assert.equal(passFinalHistory[5]?.from, "REVIEW");
assert.equal(passFinalHistory[5]?.to, "DONE");
...
}, passGuard, passStore), /Testing phase can only handle a task in TESTING status/);
//
// REWORK FLOW
//
// GIT_READY -> CODING -> IMPLEMENTED -> TESTING
...
const reworkTask = createTask({
    ...
    description: "Verify in-scope testing failure creates a fix-required state.",
    status: "GIT_READY",
    scope: {
        files: [
            "src/auth/login.ts",
...
assert.equal(reworkHistory.length, 5);
assert.equal(reworkHistory[3]?.from, "TESTING");
assert.equal(reworkHistory[3]?.to, "TRIAGE");
assert.equal(reworkHistory[4]?.from, "TRIAGE");
assert.equal(reworkHistory[4]?.to, "FIX_REQUIRED");
//
// NEW TASK FLOW
//
// GIT_READY -> CODING -> IMPLEMENTED -> TESTING
...
assert.equal(childHistory.length, 6);
assert.equal(childHistory[3]?.from, "TESTING");
assert.equal(childHistory[3]?.to, "TRIAGE");
assert.equal(childHistory[4]?.from, "TRIAGE");
assert.equal(childHistory[4]?.to, "REVIEW");
assert.equal(childHistory[5]?.from, "REVIEW");
assert.equal(childHistory[5]?.to, "DONE");
//
// INVALID START
//
...
assert.throws(() => runCodingPhase(invalidTask, invalidGuard), /Coding phase can only start for a task in GIT_READY status/);
//
...
const missingGuard = new WorkflowGuard(missingStore);
...
assert.throws(() => runCodingPhase(missingTask, missingGuard), /Task TASK-RUNNER-MISSING does not exist/);
