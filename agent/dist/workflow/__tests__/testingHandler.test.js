...
assert.equal(childHistory[1]?.reason, "Testing failed due to an out-of-scope issue; original task moved to REVIEW.");
assert.equal(childHistory[2]?.from, "REVIEW");
assert.equal(childHistory[2]?.to, "DONE");
assert.equal(childHistory[2]?.reason, "Review approved; original task completed.");
