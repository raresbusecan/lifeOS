...
const baseTask = createTask({
    ...
    description: "Validate impact map integration on task.",
    scope: {
        files: [
            "src/workflow/task.ts",
        ],
...
const taskWithoutImpactMap = validateTask(baseTask);
assert.equal(taskWithoutImpactMap.valid, true);
assert.deepEqual(taskWithoutImpactMap.errors, []);
const taskWithValidImpactMap = {
    ...baseTask,
    impactMap: validImpactMap,
};
const taskWithValidImpactMapResult = validateTask(taskWithValidImpactMap);
assert.equal(taskWithValidImpactMapResult.valid, true);
assert.deepEqual(taskWithValidImpactMapResult.errors, []);
const taskWithInvalidImpactMap = {
    ...baseTask,
    impactMap: invalidConfidenceTooHigh,
};
const taskWithInvalidImpactMapResult = validateTask(taskWithInvalidImpactMap);
assert.equal(taskWithInvalidImpactMapResult.valid, false);
assert.ok(taskWithInvalidImpactMapResult.errors.some((error) => error.startsWith("impactMap:")));
console.log("Impact Map test passed");
