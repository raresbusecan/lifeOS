...
const task = createTask({
    ...
    description: "Verify workflow coordinator starts the official coding flow.",
    status: "GIT_READY",
    scope: {
        files: [
            "src/example.ts",
...
assert.equal(codingResult.task.attempts, 0);
const storedAfterCoding = coordinator.getTask(task.id);
assert.equal(storedAfterCoding.status, "TESTING");
const codingHistory = store.getHistory(task.id);
assert.equal(codingHistory.length, 3);
assert.equal(codingHistory[0]?.from, "GIT_READY");
assert.equal(codingHistory[0]?.to, "CODING");
assert.equal(codingHistory[1]?.from, "CODING");
...
const passResult = coordinator.handleTestResult(task.id, {
    ...
    details: "All required tests passed.",
});
assert.equal(passResult.result.resultType, "PASS");
assert.equal(passResult.task.id, task.id);
assert.equal(passResult.task.status, "DONE");
assert.equal(passResult.result.newTask, null);
const finalTask = coordinator.getTask(task.id);
assert.equal(finalTask.status, "DONE");
const finalHistory = store.getHistory(task.id);
assert.equal(finalHistory.length, 6);
assert.equal(finalHistory[3]?.from, "TESTING");
assert.equal(finalHistory[3]?.to, "TRIAGE");
assert.equal(finalHistory[4]?.from, "TRIAGE");
...
console.log("Workflow coordinator test passed");
