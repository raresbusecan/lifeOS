...
export async function indexSemantic(repositoryRoot, options = {}) {
    ...
    for (const file of files) {
        ...
        for (const chunk of contentResult.entry.chunks) {
            chunks++;
            processedChunks++;
            console.log(`Embedding ${processedChunks}/${totalChunks}: ${chunk.chunkId}`);
            ...
            if (cached) {
                ...
                embedding = await client.embed(chunk.content);
                if (embedding.length !== dimensions) {
                    throw new Error(`Unexpected embedding dimensions for ${chunk.chunkId}: ${embedding.length} !== ${dimensions}`);
                }
