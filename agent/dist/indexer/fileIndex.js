...
import { loadFileCache, saveFileCache, } from "../cache/fileCache.js";
import { scanRepository, } from "../repository/scanner.js";
export async function indexRepository(repositoryRoot) {
    const cacheFile = resolve(repositoryRoot, ".agent", "cache", "files", "index.json");
    const cache = loadFileCache(cacheFile);
    const scannedFiles = await scanRepository(repositoryRoot);
