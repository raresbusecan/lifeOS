...
try {
     ...
     */
    const lines = Array.from({ length: 1700 }, (_, index) => [
        `export const value${index + 1} = ${index + 1};`,
        `export function getValue${index + 1}() {`,
        `  return value${index + 1};`,
        `}`,
    ].join("\n"));
    await writeFile(resolve(repositoryRoot, "src", "test.ts"), lines.join("\n"), "utf8");
     ...
     * batch of 20 chunks.
     */
    assert.ok(first.chunks > 20, `Expected more than 20 chunks, got ${first.chunks}`);
    ...
    console.log(`Embeddings created on first run: ${first.embeddingsCreated}`);
