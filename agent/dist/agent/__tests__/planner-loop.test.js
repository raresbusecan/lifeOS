...
const repositoryRoot = await mkdtemp(resolve(tmpdir(), "lifeos-planner-loop-test-"));
try {
    ...
    const plan = await createPlan("Analyze the authentication implementation in the repository");
    assert.ok(plan.goal);
    ...
    const result = await runAgentLoop(repositoryRoot, [
        ...
        {
            role: "user",
            content: "Analyze the authentication implementation in the repository.",
        },
    ], {
        maxSteps: 12,
    });
    ...
    console.log("");
    console.log("Planner + agent loop test passed");
